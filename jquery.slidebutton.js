...
(function($){
	...
	var methods = {
		...
		destroy : function()
		{
			...
			var options = data.options;
			// remove behaviors
			$([$input[0], data.$container[0]]).off(".slideButton");
			$(document).off(".slideButton");
	...
	var handleSetup = function()
	{
		...
		if( options.resizeHandle ){
			...
			width.handle = data.$handle.outerWidth();
		}
		...
		if( options.resizeContainer ){
			...
			data.$offlabel.css({"width" : width.container, "padding-left" : width.handle});
		} else {
			width.container = data.$container.outerWidth();
		}

		data.handleRight = width.container - width.handle;
	...
	var attachEvents = function()
	{
		...
		var localMouseUp = function(e) {
			...
			$(document).off("mousemove.slideButton touchmove.slideButton", localMouseMove);
			$(document).off("mouseup.slideButton touchend.slideButton", localMouseUp);
		};

		attachToggleEvents.call(input);

		data.$container.on("mousedown.slideButton touchstart.slideButton", function(e) {
			onMouseDown.call(input, e);
			if(options.enableDrag) $(document).on("mousemove.slideButton touchmove.slideButton", localMouseMove);
			$(document).on("mouseup.slideButton touchend.slideButton", localMouseUp);
		});
	};
	...
	var onDragMove = function(e, x)
	{
		...
		data.$onlabel.css("width", pct * data.handleRight);
		data.$onspan.css("marginLeft", -(1 - pct) * data.handleRight);
		data.$offlabel.css("right", -pct * data.handleRight);
	};
	...
	var attachToggleEvents = function()
	{
		...
		$input
			.on("change.slideButton", function() {
			...
			.on("keyup.slideButton", function(e) {
		...
		if( $.isFunction(options.click) ){
			$input.on("click.slideButton", function(){
				if(data.loading) return false;
